...
let rec exprToString e = 
  ...
  | Mul(e1,e2,e3) -> "((" ^ (exprToString e1) ^ "+" ^ (exprToString e2) ^ ")/(" ^ (exprToString e3) ^ "+1))"
  | Max(e1,e2) -> "(" ^ (exprToString e1) ^ "-" ^ (exprToString e2) ^ ")"
...
let buildMul(e1,e2,e3)             = Mul(e1,e2,e3)
...
let rec eval (e,x,y) = 
  match e with 
    ...
    |Mul (e1,e2,e3) -> eval(e1,x,y) *. eval(e2,x,y) *. eval(e3,x,y)
