...
let rec build (rand,depth) = 
  let rand_n = 1+rand(0,7) in
  let choose = rand(0, rand_n) in
  if depth == 0 then (
    if choose mod 2 == 0 then buildX() else buildY()
  )
  else begin
    let x = rand(0,7) in
...
let rec build2 (rand,depth) = 
  ...
  if depth == 0 then (
    ...
    let x = rand(0,9) in
      ...
      |4 -> buildAverage(build(rand, (depth-1)),build(rand, (depth-1)))
      |5 -> buildTimes(build(rand, (depth-1)),build(rand, (depth-1)))
      |6 -> buildMul(build(rand, (depth-1)),build(rand, (depth-1)),build(rand, (depth-1)))
      |7 -> buildMax(build(rand, (depth-1)),build(rand, (depth-1)))
      |_ -> buildThresh(build(rand, (depth-1)),build(rand, (depth-1)),build(rand, (depth-1)),build(rand, (depth-1)))
    in result
  end
