...
"""
Decorator traced. 
When the decorated function is called, the decorator should print out an ASCII art tree of the recursive calls and their return values. The format of the tree should be as follows:
...
class traced(object):
    ...
    def __call__(self,*args,**kargs):
        ...
        out += ", ".join([str(key) + "=" + repr(value) for key,value in kargs.items()])
        print("| " * traced.count + ",_"+ self.__name__ + "("+ out + ')')
        traced.count += 1
        try:
            res = self.__f(*args, **kargs)
            traced.count -= 1
            print("| " * traced.count + "`- " + repr(res))
            return res
        except Exception, e:
            traced.count -= 1
            raise e
'''
memoized decorator. 
When the decorated function is called, the decorator should check to see if the function has already been called with the given arguments. 
...
class memoized(object):
    def __init__(self,f):
        ...
        self.__f = f
    def __call__(self, *args, **kargs):
        argments = str(args) + str(kargs)
        if argments not in self.__memory:
            try:
                self.__memory[argments] = self.__f(*args,**kargs)
            except Exception as exception:
                self.__memory[argments] = exception

        if isinstance(self.__memory[argments], Exception):
            raise self.__memory[argments]
        return self.__memory[argments]
